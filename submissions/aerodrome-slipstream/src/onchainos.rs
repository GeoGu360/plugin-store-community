...
/// Resolve the wallet address for chain_id 8453 (Base) from the onchainos CLI.
/// Uses `onchainos wallet addresses` and parses data.evm[].address matching chainIndex "8453".
pub fn resolve_wallet(chain_id: u64) -> anyhow::Result<String> {
    ...
    if let Some(evm_list) = json["data"]["evm"].as_array() {
        ...
        // Fallback: return first EVM address if chain not found in list
