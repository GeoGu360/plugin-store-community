...
pub async fn run(args: RemoveLiquidityArgs) -> anyhow::Result<()> {
    ...
    let decrease_result =
        wallet_contract_call(args.chain, nfpm, &decrease_calldata, None, None, true, args.dry_run)
            .await?;
    ...
    let collect_result =
        wallet_contract_call(args.chain, nfpm, &collect_calldata, None, None, true, args.dry_run)
            .await?;
    ...
    if args.burn && liquidity_to_remove >= pos.liquidity {
        ...
        let burn_result =
            wallet_contract_call(args.chain, nfpm, &burn_calldata, None, None, true, args.dry_run)
                .await?;
