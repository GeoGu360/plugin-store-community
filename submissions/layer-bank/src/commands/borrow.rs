// src/commands/borrow.rs — Borrow an asset from LayerBank (DRY-RUN ONLY per GUARDRAILS)
//
// LayerBank Core.borrow(address lToken, uint256 amount) external
// Selector: 0x4b8a3529
//
// Requires collateral to be supplied first. DRY-RUN only per test policy.
use anyhow::Result;
...
pub async fn run(
    ...
    // borrow(address lToken, uint256 amount) selector = 0x4b8a3529
    let ltoken_padded = format!("{:0>64}", market.ltoken.trim_start_matches("0x"));
    let amount_padded = format!("{:064x}", raw_amount);
    let calldata = format!("0x4b8a3529{}{}", ltoken_padded, amount_padded);

    if dry_run {
        return Ok(json!({
            "ok": true,
            "dry_run": true,
            "action": format!("borrow {} from LayerBank", asset),
            ...
            "steps": [
                {
                    "step": 1,
                    "action": format!("Core.borrow(l{}, {})", asset, raw_amount),
                    "to": CORE,
                    "calldata": calldata.clone()
                }
            ],
            "warning": "Borrow creates debt with liquidation risk. Ask user to confirm before executing."
        }));
    }
    ...
    Ok(json!({
        ...
        "warning": "Ask user to confirm before broadcasting. Borrow creates liquidation risk.",
