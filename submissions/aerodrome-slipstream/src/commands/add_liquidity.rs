...
use crate::config::{
    build_approve_calldata, encode_tick, factory_address, nfpm_address, pad_address, pad_u256,
    resolve_token_address, rpc_url, unix_now,
};
...
pub struct AddLiquidityArgs {
    ...
    #[arg(long)]
    pub token1: String,
    /// Tick spacing (1/50/100/200/2000) — Slipstream uses tickSpacing not fee
    #[arg(long)]
    pub tick_spacing: i32,
    /// Lower tick of the position (can be negative, e.g. -887272)
    #[arg(long, allow_hyphen_values = true)]
    pub tick_lower: i32,
    /// Upper tick of the position (can be negative)
    #[arg(long, allow_hyphen_values = true)]
    pub tick_upper: i32,
...
pub async fn run(args: AddLiquidityArgs) -> anyhow::Result<()> {
    ...
    let pool_addr = factory_get_pool(&token0, &token1, args.tick_spacing, factory, rpc).await?;
    if pool_addr == "0x0000000000000000000000000000000000000000" {
        anyhow::bail!(
            "Pool does not exist for {}/{} tickSpacing={}. Deploy the pool first.",
            token0,
            token1,
            args.tick_spacing
        );
    ...
    println!(
        "Adding liquidity: {}/{} tickSpacing={} tickLower={} tickUpper={} amount0={} amount1={}",
        token0, token1, args.tick_spacing, args.tick_lower, args.tick_upper,
    ...
    if !args.dry_run {
        ...
        if allowance0 < args.amount0_desired {
            ...
            let res = wallet_contract_call(CHAIN_ID, &token0, &approve_data, true, false).await?;
        ...
        if allowance1 < args.amount1_desired {
            ...
            let approve_data = build_approve_calldata(nfpm, u128::MAX);
            let res = wallet_contract_call(CHAIN_ID, &token1, &approve_data, true, false).await?;
    ...
    // Aerodrome Slipstream MintParams:
    // (token0, token1, tickSpacing, tickLower, tickUpper,
    //  amount0Desired, amount1Desired, amount0Min, amount1Min,
    //  recipient, deadline, sqrtPriceX96)
    // Selector: 0xb5007d1f
    // CRITICAL: Slipstream adds sqrtPriceX96 at the end (for new pool init). Set to 0 if pool exists.
    let deadline = unix_now() + args.deadline_minutes * 60;
    // tickSpacing for mint is int24, encode same as ticks
    let ts_hex = encode_tick(args.tick_spacing);
    let calldata = format!(
        "0xb5007d1f{}{}{}{}{}{}{}{}{}{}{}{}",
        pad_address(&token0),
        pad_address(&token1),
        ...
        pad_u256(0), // sqrtPriceX96 = 0 (pool already exists)
    );

    let result = wallet_contract_call(CHAIN_ID, nfpm, &calldata, true, args.dry_run).await?;

    let tx_hash = extract_tx_hash(&result);
    println!(
        "{{\"ok\":true,\"txHash\":\"{}\",\"token0\":\"{}\",\"token1\":\"{}\",\"tickSpacing\":{},\"tickLower\":{},\"tickUpper\":{}}}",
        tx_hash, token0, token1, args.tick_spacing, args.tick_lower, args.tick_upper
    );
