...
use commands::{
    add_liquidity::AddLiquidityArgs, collect_fees::CollectFeesArgs, get_pools::GetPoolsArgs,
    get_positions::GetPositionsArgs, quote::QuoteArgs, remove_liquidity::RemoveLiquidityArgs,
    swap::SwapArgs,
};

#[derive(Parser)]
#[command(name = "aerodrome-slipstream", version, about = "Aerodrome Slipstream CLMM Plugin for Base")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
#[derive(Subcommand)]
enum Commands {
    /// Get a swap quote via QuoterV2 (no transaction)
    Quote(QuoteArgs),
    /// Swap tokens via SwapRouter exactInputSingle
    Swap(SwapArgs),
    /// Add concentrated liquidity (mint a new position)
    ...
    /// Query pool addresses from CLFactory
