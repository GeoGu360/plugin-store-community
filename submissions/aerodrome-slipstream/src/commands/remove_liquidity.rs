...
pub struct RemoveLiquidityArgs {
    ...
    #[arg(long)]
    pub token_id: u128,
    /// Amount of liquidity to remove. If omitted, removes all liquidity.
    #[arg(long)]
    ...
    /// Burn the NFT after removing all liquidity
    #[arg(long)]
    pub burn: bool,
...
pub async fn run(args: RemoveLiquidityArgs) -> anyhow::Result<()> {
    ...
    println!(
        "Position #{}: token0={} token1={} tickSpacing={} liquidity={}",
        args.token_id, pos.token0, pos.token1, pos.tick_spacing, pos.liquidity
    );

    let liquidity_to_remove = args.liquidity.unwrap_or(pos.liquidity);
    if liquidity_to_remove == 0 && pos.liquidity == 0 {
        println!("Position has no liquidity. Attempting collect to sweep any remaining fees...");
    }
    ...
    let decrease_result =
        wallet_contract_call(CHAIN_ID, nfpm, &decrease_calldata, true, args.dry_run).await?;
    let decrease_tx = extract_tx_hash(&decrease_result).to_string();
    println!("decreaseLiquidity tx: {}", decrease_tx);

    // Wait 5s for decreaseLiquidity nonce to clear before collect
    if !args.dry_run {
    ...
    let collect_result =
        wallet_contract_call(CHAIN_ID, nfpm, &collect_calldata, true, args.dry_run).await?;
    ...
    if args.burn && liquidity_to_remove >= pos.liquidity {
        ...
        let burn_result =
            wallet_contract_call(CHAIN_ID, nfpm, &burn_calldata, true, args.dry_run).await?;
