...
pub async fn run(args: CollectFeesArgs) -> anyhow::Result<()> {
    ...
    // --- 1. Fetch position to display fee info ---
    let pos = nfpm_positions(nfpm, args.token_id, rpc).await?;
    println!(
        "Position #{}: token0={} token1={} tickSpacing={} tokensOwed0={} tokensOwed1={}",
        args.token_id, pos.token0, pos.token1, pos.tick_spacing,
        pos.tokens_owed0, pos.tokens_owed1
    );
    ...
    // --- 2. Resolve recipient (must not be zero address for real collect) ---
