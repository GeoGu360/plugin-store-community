...
use crate::config::{
    ...
    ROUTER_V2, ETH_RPC, CHAIN_ID, FACTORY_V2,
};
...
///   2. token + ETH    → removeLiquidityETH
///
...
pub async fn run(
    ...
    if a_is_eth && b_is_eth {
        anyhow::bail!("Cannot remove ETH + ETH liquidity");
    }
    ...
    let (amount_a_min, amount_b_min) = if !dry_run {
        ...
        let expected_a = if total_supply > 0 {
            lp_balance.saturating_mul(reserve_a) / total_supply
        } else {
            0
        };
        let expected_b = if total_supply > 0 {
            lp_balance.saturating_mul(reserve_b) / total_supply
        } else {
            0
        };
    ...
    if a_is_eth || b_is_eth {
        ...
        // removeLiquidityETH
        // Selector: 0x02751cec
        // Params: (address token, uint256 liquidity, uint256 amountTokenMin,
        //          uint256 amountETHMin, address to, uint256 deadline)
        // ---------------------------------------------------------------
        let (token_addr, token_sym, amount_token_min, amount_eth_min) = if b_is_eth {
        ...
        println!("Remove Liquidity ETH: {} / ETH", token_sym.to_uppercase());
        ...
        // Selector: 0xbaa2abde
        // Params: (address tokenA, address tokenB, uint256 liquidity,
