// src/onchainos.rs — onchainos CLI wrappers for LayerBank
use std::process::Command;
use serde_json::Value;

/// Query the currently logged-in EVM wallet address.
/// Searches the evm list for the given chain_id, falls back to first entry.
...
/// ⚠️  dry_run=true → early-return simulated response; never pass --dry-run to onchainos.
/// ⚠️  --force is always included so the tx is actually broadcast.
pub async fn wallet_contract_call(
    ...
    let mut args: Vec<String> = vec![
        ...
        "--input-data".into(), input_data.to_string(),
        "--force".into(),
    ];
    if let Some(v) = amt {
        args.push("--amt".into());
