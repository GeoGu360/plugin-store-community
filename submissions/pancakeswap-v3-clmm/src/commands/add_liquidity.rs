...
pub struct AddLiquidityArgs {
    ...
    #[arg(long)]
    pub token1: String,
    /// Fee tier (100/500/2500/10000)
    #[arg(long)]
    pub fee: u32,
    /// Lower tick of the position (can be negative)
...
pub async fn run(args: AddLiquidityArgs) -> anyhow::Result<()> {
    ...
    if !args.dry_run {
        ...
        if allowance0 < args.amount0_desired {
            ...
            let res =
                wallet_contract_call(args.chain, &token0, &approve_data, None, None, true, false)
                    .await?;
        ...
        if allowance1 < args.amount1_desired {
            ...
            let approve_data = build_approve_calldata(nfpm, u128::MAX);
            let res =
                wallet_contract_call(args.chain, &token1, &approve_data, None, None, true, false)
                    .await?;
