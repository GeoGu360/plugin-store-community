...
use crate::config::{
    ...
    resolve_token_address, rpc_url, smart_router, unix_now,
};
...
pub struct SwapArgs {
    ...
    /// Amount in (smallest token unit, e.g. wei)
    #[arg(long)]
    pub amount_in: u128,
    /// Slippage tolerance in percent (e.g. 0.5 = 0.5%)
    #[arg(long, default_value = "0.5")]
    pub slippage: f64,
    /// Fee tier (100/500/2500/10000). If omitted, auto-selects best.
    #[arg(long)]
    pub fee: Option<u32>,
...
pub async fn run(args: SwapArgs) -> anyhow::Result<()> {
    ...
    let router = smart_router(args.chain)?;
    ...
    if !args.dry_run {
        ...
        if allowance < args.amount_in {
            println!("Approving {} for SmartRouter...", token_in);
            let approve_data = build_approve_calldata(router, u128::MAX);
            let approve_result =
                wallet_contract_call(args.chain, &token_in, &approve_data, None, None, true, false)
                    .await?;
    ...
    let calldata = format!(
        ...
        pad_u256(0), // sqrtPriceLimitX96 = 0
    );

    let result =
        wallet_contract_call(args.chain, router, &calldata, None, None, true, args.dry_run)
            .await?;
